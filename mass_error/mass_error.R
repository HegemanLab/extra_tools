...
# by comparing observed and known masses of chemicals.
#
# The input file must contain the columns:
# (1) chemical (2) formula (3) known_mass & (4) observed mass.

df <- read.table('input-data.tsv',header=T)

df$delta <- df$known_mass-df$observed_mass
df$mass_error <- df$delta/df$known_mass*1e6
...
summary <- aggregate(cbind(observed_mass, mass_error) ~ known_mass, df, mean)
summary$delta <- summary$known_mass - summary$observed_mass
summary$chemical <- as.character(unique(df$chemical))
...
summary <- subset(summary, select=c(5,6,1,2,4,3))
write.table(summary,file='output-summary.tsv',sep="\t",row.names=F)

#install.packages("ggplot2",repos = "http://cran.us.r-project.org")
library(ggplot2)
p1 <- ggplot(df, aes(known_mass, mass_error)) +
      geom_boxplot(aes(group = known_mass)) +
...
#p2 <- ggplot(df, aes(known_mass, delta)) + geom_boxplot(aes(group = known_mass)) + expand_limits(x=0,y=0)
